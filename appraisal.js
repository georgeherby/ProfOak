...
module.exports = {
    ...
    getCoreDetailsFromUser: function (client, message, pokemonNameArray, pokemonIdArray) {
        ...
        if (utils.isPokemon(pokemon, pokemonNameArray)){
            getPokemonIv(message, pokemonNameArray,pokemonIdArray, stardust, hp, cp, pokemon, poweredup)
        }else{
...
function getPokemonIv (message, pokemonNameArray,pokemonIdArray, stardust, hp,cp,pokemon, poweredUp) {
    let maxSta = 15;
    let maxDef = 15;
    let maxAtk = 15;
    ...
    connection.query(query, function(err, results){
       let potentialLevels = levelsByDust(results[0]);
        ...
        let embed = new Discord.RichEmbed().setTitle(`**Possible IV For ${utils.toTitleCase(pokemon)}**`);
        embed.setThumbnail(utils.getThumbnail(utils.getDexIdFromName(pokemon,pokemonNameArray,pokemonIdArray)));
        ...
        if(potentialIVs.length > 0) {
            ...
            embed.setDescription('```No possible IVs for the information supplied...```');
        }
...
function testCP(cp, atkIV, defIV, staIV, levelData, pokemonStats) {
    ...
    let calcCP = parseInt(attackFactor * defenseFactor * staminaFactor * scalarFactor / 10, 10);
